...
// error messages
var (
	errStructField           = errors.New("orm: field %s does not exist in %s")
	errSelfReference         = errors.New("orm: self reference is only allowed on many to many")
	errRelationKind          = errors.New("orm: %s.%s relation kind %s is not allowed on %s")
	errJoinFK                = errors.New("orm: fk field %s or association fk field %s does not exit in table %s")
	errPolymorphicNotAllowed = errors.New("orm: %s - polymorphic is not allowed on relation type %s")
)
...
type Relation struct {
	// Type for easier reflection - TODO experimental if needed
	Type reflect.Type
...
func (m Model) foreignKey(tag string) (Field, error) {
	...
	if tag != "" {
		return scope.Field(tag)
	}

	return scope.PrimaryKeys()[0], nil // TODO change if more than one pk is allowed
}

func polymorphicRestriction(tags map[string]string, field string, rel string) error {
	if _, ok := tags[tagPolymorphic]; ok {
		return fmt.Errorf(errPolymorphicNotAllowed.Error(), field, rel)
	}
...
func (m Model) polymorphic(tags map[string]string, rel Interface) (Polymorphic, error) {
	if v, ok := tags[tagPolymorphic]; ok {
		scope := NewScopeFromInterface(rel)
		// {name}ID
		f, err := scope.Field(v + "ID")
		if err != nil {
			return Polymorphic{}, err
		}
		// {name}Type
		t, err := scope.Field(v + "Type")
		if err != nil {
			return Polymorphic{}, err
		}
		// value
...
// Error will return if the field does not exist.
func (m Model) associationForeignKey(tag string, rel Interface) (Field, error) {
	if tag == "" {
		tag = m.modelName(false) + "ID"
	}
	return NewScopeFromInterface(rel).Field(tag)
}
...
func joinTable(m Interface, rel Interface, tags map[string]string, selfRef bool) (JoinTable, error) {
	...
	if joinTable == "" {
		joinTable = strings.CamelToSnake(strings.Plural(m.modelName(false) + rel.modelName(false)))
	}
	if joinFK == "" {
		fk, _ := m.foreignKey(tags[tagForeignKey])
		joinFK = strings.CamelToSnake(m.modelName(false) + fk.Name)
	}
	if joinAFK == "" {
		if !selfRef {
			afk, _ := rel.foreignKey(tags[tagAssociationForeignKey])
			joinAFK = strings.CamelToSnake(rel.modelName(false) + afk.Name)
		} else {
	...
	if len(cols) != 2 {
		return JoinTable{}, fmt.Errorf(errJoinFK.Error(), joinFK, joinAFK, joinTable)
	}
...
func (m *Model) createRelations() error {
	...
	for _, relation := range m.structFields(m.caller, true) {
		...
		if _, ok := tags[tagCustom]; ok {
			m.relations = append(m.relations, Relation{Kind: rel, SelfReference: false, Field: relation.Name, Custom: true})
			continue
		}
		...
		case HasOne, HasMany:
			...
			m.relations = append(m.relations, Relation{Type: v.Type(), Permission: defaultPermission, Kind: rel, SelfReference: selfRef, Field: relation.Name, ForeignKey: fk, Polymorphic: poly, AssociationForeignKey: afk})
		case BelongsTo:
			...
			fk, err := rModel.associationForeignKey(tags[tagForeignKey], m)
			if err != nil {
				return err
			}
			afk, err := rModel.foreignKey(tags[tagAssociationForeignKey])
			if err != nil {
				return err
			}

			m.relations = append(m.relations, Relation{Type: v.Type(), Permission: defaultPermission, Kind: rel, SelfReference: selfRef, Field: relation.Name, ForeignKey: fk, AssociationForeignKey: afk})
		case ManyToMany:
			// Polymorphic is not allowed on m2m
			err = polymorphicRestriction(tags, relation.Name, rel)
			if err != nil {
			...
			m.relations = append(m.relations, Relation{Type: v.Type(), Permission: defaultPermission, Kind: rel, SelfReference: selfRef, Field: relation.Name, ForeignKey: fk, AssociationForeignKey: afk, JoinTable: jTable})
		}
	}
...
func (m Model) relationKind(tags map[string]string, field reflect.StructField) (string, error) {
	...
	if tag, ok := tags[tagRelation]; ok {
		if !isTagRelationAllowed(field, tag) {
			return "", fmt.Errorf(errRelationKind.Error(), m.name, field.Name, tag, field.Type.Kind())
		}
		return tag, nil
	}
	...
	return "", fmt.Errorf(errRelationKind.Error(), m.name, field.Name, field, field.Type.Kind())
}
...
func (m Model) initializeModelByValue(r reflect.Value) (Interface, error) {
	exist := m.initRelation(r.Type().String())
	if exist != nil {
		return exist, nil
	}
	...
	// passing parent cache to child
	c, ttl, err := m.Cache()
	if err != nil {
		return nil, err
	}
	rel.setCaller(rel) //needed otherwise the model name (cache name) is empty
	err = rel.SetCache(c, ttl)
	if err != nil {
		return nil, err
	}

	// set initialized relations to avoid loops
	rel.setInitRelations(m.initRel)
