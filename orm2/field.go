...
const (
	model = "orm2.Model" // used to identify the embedded orm field.
...
var (
	errDBColumn   = errors.New("orm: column %s does not exist table %s")
	errDbSync     = errors.New("orm: database is not in sync with the struct")
	errPrimaryKey = errors.New("orm: no primary key or ID FIELD is set in  %s")
)
...
func (m Model) checkPrimaryKey() error {
	...
	if pkCounter == 0 {
		if IDField == nil {
			return fmt.Errorf(errPrimaryKey.Error(), m.modelName(true))
		}
...
Columns:
	for i := 0; i < len(m.fields); i++ {
		...
		// if the predefined time fields does not exist in the database, delete it.
		...
		return fmt.Errorf(errDBColumn.Error(), m.fields[i].Information.Name, m.caller.DefaultTableName())
	}
