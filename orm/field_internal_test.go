...
func (c carNull) DefaultTableName() string {
	return "cars"
}
...
func TestModel_createFieldsNoPrimary(t *testing.T) {
	...
	err := c.Init(c)
	test.Error(err)
	test.Equal(fmt.Sprintf(errPrimaryKey, c.modelName(true)), err.Error())
}

// test - the db field is null able but the struct field is a normal string.
func TestModel_carTypeNull(t *testing.T) {
	test := assert.New(t)
	c := &carNull{}
	err := c.Init(c)
	test.Error(err)
	test.Equal(fmt.Sprintf(errNullField, "orm_test.cars", "type", "Type"), err.Error())
}
...
func TestModel_carFieldDoesNotExist(t *testing.T) {
	...
	err := c.Init(c)
	test.Error(err)
	test.Equal(fmt.Sprintf(errDBColumn, "Engine", "cars.engine"), err.Error())
}
...
func TestModel_createFields(t *testing.T) {
	...
	if assert.Equal(t, 9, len(c.fields)) {
		// table driven tests
		var tests = []struct {
			Name        string
			...
			{Name: "CreatedAt", Information: sqlquery.Column{Table: "cars", Name: "created_at", PrimaryKey: false}, Permission: Permission{Read: false, Write: true}, custom: false, sql: ""},
			{Name: "UpdatedAt", Information: sqlquery.Column{Table: "cars", Name: "updated_at", PrimaryKey: false}, Permission: Permission{Read: false, Write: true}, custom: false, sql: ""},
		}
