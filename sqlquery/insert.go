...
// Insert provides some features for a sql insert
type Insert struct {
	builder     *Builder
	transaction bool
...
func (s *Insert) render() (stmt string, args [][]interface{}, err error) {
	...
	if len(s.valueSets) == 0 {
		return "", [][]interface{}{}, errors.New("sql: no insert value is set")
	}
	...
	selectStmt := "INSERT INTO " + s.builder.QuoteIdentifier(s.into) + "(" + s.builder.escapeColumns(s.columns) + ") VALUES "
	...
	return s.builder.replacePlaceholders(selectStmt), s.arguments, nil
}
...
func (s *Insert) Exec() ([]sql.Result, error) {
	...
	if s.lastIDPtr != nil {
		return nil, s.execQueryWithLastId(stmt, args, nil)
	}
...
// An error will return if the arguments and placeholders mismatch or no value was set.
func (s *Insert) ExecTx(tx *sql.Tx) ([]sql.Result, error) {
	stmt, args, err := s.render()
	if err != nil {
	...
	if s.lastIDPtr != nil {
		return nil, s.execQueryWithLastId(stmt, args, tx)
	}

	return s.builder.execTx(tx, stmt, args)
}
...
func (s *Insert) execQueryWithLastId(stmt string, args [][]interface{}, tx *sql.Tx) error {
	var err error
	if s.builder.conf.Driver() == "postgres" {
		stmt = stmt + " RETURNING " + s.builder.QuoteIdentifier(s.lastIDColumn)
		var row *sql.Row
		if tx == nil {
			row, err = s.builder.first(stmt, args[0])
		} else {
			row, err = s.builder.firstTx(tx, stmt, args[0])
		}
		if err != nil {
			return err
		}
		...
		if tx == nil {
			...
			res, err = s.builder.execTx(tx, stmt, args)
		}
		if err != nil {
			return err
		}
